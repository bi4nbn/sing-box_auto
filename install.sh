...
set_as_entrance() {
    if [[ ! -f "${SCRIPT_FILE_PATH}" ]]; then
        wget --no-check-certificate -O ${SCRIPT_FILE_PATH} https://raw.githubusercontent.com/bi4nbn/sing-box_auto/refs/heads/main/install.sh
...
download_config() {
    ...
    if [[ ! -f "${CONFIG_FILE_PATH}/config.json" ]]; then
        wget --no-check-certificate -O ${CONFIG_FILE_PATH}/config.json https://raw.githubusercontent.com/bi4nbn/sing-box_auto/main/server.json
        if [[ $? -ne 0 ]]; then
